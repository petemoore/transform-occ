...
$client.DownloadFile("https://www.mercurial-scm.org/release/windows/mercurial-4.3.3-x64.msi", "C:\binaries\7.msi")
...
# fakemon
$client.DownloadFile("https://raw.githubusercontent.com/mozilla-releng/OpenCloudConfig/master/userdata/Configuration/fakemon.vbs", "C:\Windows\System32\fakemon.vbs")

# configmymonitor
$client.DownloadFile("https://s3.amazonaws.com/windows-opencloudconfig-packages/fakemon/configmymonitor.exe", "C:\Windows\System32\configmymonitor.exe")
...
Start-Process "powershell" -ArgumentList "-command `"& {& (Get-Acl -Path 'HKLM:\SOFTWARE\Mozilla').SetAccessRule(New-Object System.Security.AccessControl.RegistryAccessRule ('Everyone', 'FullControl', 'Allow'))}`"; `"& {& ((Get-Acl -Path 'HKLM:\SOFTWARE\Mozilla') | Set-Acl -Path 'HKLM:\SOFTWARE\Mozilla')}`"" -Wait -NoNewWindow
...
# NvidiaK520DriverPackage: Required for hardware acceleration on EC2 g2 (GPU) instances. http://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/accelerated-computing-instances.html#install-nvidia-driver-windows
$client.DownloadFile("http://uk.download.nvidia.com/Windows/Quadro_Certified/GRID/369.95/369.95-quadro-grid-desktop-notebook-win10-64bit-international-whql.exe", "C:\binaries\8.exe")
Start-Process "C:\binaries\8.exe" -ArgumentList "/s" -Wait -NoNewWindow

# NvidiaK520DriverInstall: Required for hardware acceleration on EC2 g2 (GPU) instances. http://docs.aws.amazon.com/AWSEC2/latest/WindowsGuide/accelerated-computing-instances.html#install-nvidia-driver-windows
Start-Process "C:\NVIDIA\369.95\setup.exe" -ArgumentList "/s" -Wait -NoNewWindow

# reg_DisableTouchEvents: Bug 1382988 - Disable touch events
New-ItemProperty -Path "HKLM:SOFTWARE\Microsoft\Wisp\Touch" -Name "TouchGate" -Value "0x00000000" -PropertyType Dword -Force
...
# AssocTxtfileNotepad: Bug 1373551 - associate text files with notepad
Start-Process "ftype" -ArgumentList "txtfile=`"C:\Windows\System32\Notepad.exe`" `"%1`"" -Wait -NoNewWindow

# UltraVNC: VNC Server - not essential for building firefox
$client.DownloadFile("https://s3.amazonaws.com/windows-opencloudconfig-packages/UltraVnc/UltraVnc_10962_x64.msi", "C:\binaries\9.msi")
Start-Process "msiexec" -ArgumentList "/i C:\binaries\9.msi /quiet" -Wait -NoNewWindow
...
New-ItemProperty -Path "HKLM:SOFTWARE\Policies\Microsoft\Windows\Explorer" -Name "NoNewAppAlert" -Value "0x00000001" -PropertyType Dword -Force
